...
function selectYahooBase(timeframes, historyDays) {
  ...
  const tfList = Array.isArray(timeframes) ? timeframes : (timeframes ? [timeframes] : ['1d']);
  const finest = ORDER.find(iv => tfList.includes(iv));
...
async function loadOptionsConfig() {
  ...
  try {
    ...
    if (err.code !== 'ENOENT') {
      console.warn(`[INGEST] Warning: Unable to read options config: ${err.message}`);
    }
  }
...
function removeRejectedSources(snapshot, rejectStale = true) {
  ...
  if (!report || !Array.isArray(report.rejected_keys) || report.rejected_keys.length === 0) {
    return { removed_records: 0, stale_records: report?.freshness?.stale_records || 0 };
  }
...
function aggregateCandles(candles, interval, symbol, provider, family = "unknown") {
  ...
  for (const candle of candles) {
    ...
    const openTime = Number.isFinite(candle.openTime)
      ? Number(candle.openTime)
      : (candle.timestamp ? new Date(candle.timestamp).getTime() : NaN);
...
function dedupePreferredMarketQuotes(records) {
  if (!Array.isArray(records)) {
    return { records: [], input_records: 0, quote_records: 0, removed_records: 0 };
  }
  const quoteRecords = [];
  const otherRecords = [];
...
function selectNearestOptions(contracts, spot, count = 1) {
  ...
  return [...contracts]
    .filter((contract) => contract && Number.isFinite(Number(contract.strike)))
    .sort((a, b) => {
...
async function fetchEquityOrIndexSnapshot(family, provider, symbol, timeframes, config, options = {}) {
  ...
  if (provider === 'stooq') {
    ...
    baseCandles = await withRetry(() => fetchStooqDailyHistory(stooqSymbol));
  } else {
    ...
    baseCandles = await withRetry(() => fetchYahooBaseCandles(providerSymbol, bestBase, effectiveDays, startTime, endTime));
  }
  const output = [];

  for (const timeframe of (timeframes || ['1d'])) {
...
async function fetchCommoditySnapshot(family, provider, symbol, timeframes, config, options = {}) {
  ...
  if (provider === 'stooq') {
    ...
    baseCandles = await withRetry(() => fetchYahooBaseCandles(providerSymbol, bestBase, historyDays, startTime, endTime));
  }
  const output = [];
...
async function fetchYahooOptionsSnapshot(family, provider, underlying) {
  if (provider === 'cboe') {
    ...
    const payload = await withRetry(() => fetchJson(url.toString()));
  ...
  const underlyingCandles = await withRetry(() => fetchYahooBaseCandles(underlying));
  if (!Array.isArray(underlyingCandles) || underlyingCandles.length === 0) {
    throw new Error(`Unable to determine spot price for ${underlying}: no candles returned`);
  }
  const lastCandle = underlyingCandles[underlyingCandles.length - 1];
  const spot = Number(lastCandle?.close);
  ...
  const expiration = Number.isFinite(Number(expirationDate))
    ? new Date(Number(expirationDate) * 1000).toISOString()
