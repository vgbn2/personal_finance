...
function parseStrategyYaml(text) {
  ...
  function list(key) {
    for (let i = 0; i < lines.length; i += 1) {
      ...
      for (let c = i + 1; c < lines.length; c += 1) {
        if (!lines[c].trim()) continue;
        if (lines[c].match(/^\s*/)[0].length <= base) break;
        const item = lines[c].trim().match(/^-\s*(.+)$/);
  ...
  function section(key) {
    for (let i = 0; i < lines.length; i += 1) {
      ...
      for (let c = i + 1; c < lines.length; c += 1) {
        if (!lines[c].trim()) continue;
        if (lines[c].match(/^\s*/)[0].length <= base) break;
        const kv = lines[c].match(/^\s+([^:]+):\s*(.+)$/);
        if (!kv) continue;
        const v = kv[2].trim().replace(/^["']|["']$/g, '');
        out[kv[1].trim()] = v === 'true' ? true : v === 'false' ? false : (!Number.isNaN(Number(v)) && v !== '' ? Number(v) : v);
      }
      return out;
    }
    return {};
  ...
  function featureLists() {
    for (let i = 0; i < lines.length; i += 1) {
      const m = lines[i].match(/^(\s*)features:\s*$/);
      if (!m) continue;
      const base = m[1].length;
      const out = { technical: [], relative: [], orderflow: [], custom: [] };
      let cur = null;
      for (let c = i + 1; c < lines.length; c += 1) {
        if (!lines[c].trim()) continue;
        const ind = lines[c].match(/^\s*/)[0].length;
        if (ind <= base) break;
        const sec = lines[c].match(/^\s+([^:]+):\s*$/);
        if (sec && ind === base + 2) { cur = sec[1].trim(); if (!out[cur]) out[cur] = []; continue; }
        const item = lines[c].match(/^\s+-\s*(.+)$/);
        if (item && cur) out[cur].push(item[1].replace(/^["']|["']$/g, '').trim());
      }
      return out;
    }
    return { technical: [], relative: [], orderflow: [], custom: [] };
  }
  ...
  return {
    ...
    model:              scalar('model'),
    timeframe:          scalar('timeframe') || null,
    universe:           list('universe'),
    risk:               section('risk'),
    indicators:         section('indicators'),
    indicator_periods:  section('indicator_periods'),
...
async function runAutomationPass(args, strategiesOverride = null) {
    ...
    for (const strategy of targetStrategies) {
        ...
        if (report && report.trades && report.trades.length > 0) {
            ...
            const allocationUsd = totalEquity * riskWeight;
